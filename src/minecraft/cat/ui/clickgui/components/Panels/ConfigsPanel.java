...
public class ConfigsPanel extends Panel {
    ...
    public void drawPanel(int mouseX, int mouseY, float partialTicks, boolean handleClicks) {
        ...
        for (File file : files) {
            ...
            f.drawString(no, x + 5, y + (mHeight / 2f - f.FONT_HEIGHT / 2f), selectedConfig != null && selectedConfig.equals(no) ? main_color.getRGB() : main_color.darker().darker().getRGB());
        ...
        f.drawString(val.name + ": "+val.get() + (selectedTextField != null && selectedTextField == val && textFieldCounter > partialTicks % 0.5 ? "_" : ""), x + 5, y + (20 / 2f - f.FONT_HEIGHT / 2f), Color.WHITE.getRGB());
        if (Mouse.isButtonDown(0) && i(mouseX, mouseY, x, y, x + width, y + 10) && handleClicks && !sex) {
            sex = true;
        ...
        f.drawString("Save", x + 5, y + 10 + (mHeight / 2f - f.FONT_HEIGHT / 2f), main_color.getRGB());
        if(Mouse.isButtonDown(0) && i(mouseX, mouseY, x, y  + 10, x + width, y + 30) && handleClicks && !sex) {
            update();
            sex = true;
        ...
        f.drawString("Delete", x + 5, y + 20 + (mHeight / 2f - f.FONT_HEIGHT / 2f), main_color.getRGB());
        ...
        f.drawString("Load", x + 5, y + 30 + (mHeight / 2f - f.FONT_HEIGHT / 2f), main_color.getRGB());
