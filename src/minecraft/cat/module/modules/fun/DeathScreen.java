...
public class DeathScreen extends Module {
    ...
    public void render2D(Render2DEvent e){
        ...
        if(mc.thePlayer.getHealth() <= 0){
            ...
            RenderUtil.drawImage(new ResourceLocation("cat/fun/jaja un meme.jpg"), e.resolution.getScaledWidth() / 2f - (w / 2f), ImageAnim.getValue(), w,h, 1);
            RenderUtil.drawScaledFont(FontUtil.fontComicSans42, "epic fail XDDDDD", e.resolution.getScaledWidth() / 2f - (mc.fontRendererObj.getStringWidthF("epic fail XDDDDD") * 4 / 2), ImageAnim.getValue() - (h / 4f) - (mc.fontRendererObj.FONT_HEIGHT * 4 / 2f) - 2, Color.WHITE.getRGB(),true, 4);
            RenderUtil.drawImage(new ResourceLocation("cat/fun/red.png"), e.resolution.getScaledWidth() / 2f - 101, e.resolution.getScaledHeight() - 59, 100, 50, alpha / 255f);
            drawArrows(pX, pY);
    ...
    public void drawArrows(float x, float y){
        RenderUtil.drawImage(new ResourceLocation("cat/fun/arrow.jpg"), x - 15 - arrowAnim.getValue(), y, 60, 30, 1);

        GlStateManager.pushMatrix();
        GlStateManager.translate(x + 30 - arrowAnim.getValue(), y - 15 - arrowAnim.getValue(), 0);
        GL11.glRotatef(45, 0, 0, 0.1f);
        RenderUtil.drawImage(new ResourceLocation("cat/fun/arrow.jpg"), -30, -15, 60, 30, 1);
    ...
    public void updateAndSetLimits(Render2DEvent e, float h){
        ...
        alpha = MathUtil.inRange((int) (alpha - 0.2 * RenderUtil.delta), 0, 255);
        if(alpha < 1)
            alpha = 255;
